...
class CommonPlatform ():
    PackagesSupported = ("T1Pkg")
    ArchSupported = ("AARCH64")
    TargetsSupported = ("DEBUG", "RELEASE")
    Scopes = ('t1', 'gcc_aarch64_linux', 'edk2-build')
...
class PlatformBuilder (UefiBuilder, BuildSettingsManager):
    ...
    def PlatformAppendDtb (self):
        ...
        dtb = os.path.join(blob_dir, "mt6771-mashtab-t1.dtb")
        appended_output = fd_path + ".dtb"

        try:
            with open(dtb, 'rb') as dt, \
                open(fd_path, 'rb') as fd, \
                open(appended_output, 'wb') as out_file:
                out_file.write(fd.read())
                out_file.write(dt.read())
        except Exception as e:
            logging.critical(f"Failed to append dtb: {e}")
            return 1
    ...
    def PlatformBuildBootImage (self):
        ...
        fd_path = os.path.join(self.PlatformGetOutDir(), "FV", f"{platform.upper()}_UEFI.fd.shimmed.gz.dtb")
